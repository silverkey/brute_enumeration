...
# 
# This specific script is an extension of the original which work constraining the motif
# to a specific requirement of the user in term of string composition

my $USAGE = "\n\tperl $0 [fasta file] [number of shuffling] [length of words] [minimum number of seq] [constraint (eg: TATAAA)]\n\n";
...
my $WL = $ARGV[2];
my $MIN_SEQ = $ARGV[3];
my $CONSTRAINT = $ARGV[4];

my $SWD = getcwd();
my $NWD = $SWD.'/'.$FASTA.'_NSH_'.$NUM_SH.'_WL_'.$WL.'_'.$CONSTRAINT;
$NWD =~ s/\.fa//;
...
sub count_real {
  ...
  while(my $seq = $seqio->next_seq) {
    ...
    while($start <= $last) {
      ...
      next if $substring =~ /N/;
      next unless $substring =~ /$CONSTRAINT/;
